...
print "运行日志保存到<temp/summary.txt>\n";
print "------ " . ctime() . " ------\n\n";
open SMR, ">temp/summary.txt" or die "$!";
print SMR "Script: $0\n";
...
sub report {
  ...
  my ( $filename, $stage, $note_s, $note_c );
  ...
  @raw_files = glob "temp/raw_lot*.txt";
  foreach $i ( 0 .. $#raw_files ) {
    $raw_files[$i] =~ /(lot\d+).txt/;
    $sheet = $book->add_worksheet($1);
    &setformat;
    @file = glob "temp/$1/*.gpr";
    $note_s = $#file == 0 ? '1 slide.' : ($#file+1)." slides.";
    # Raw Intensity File.xlsx
    $note = qq~# This page contains raw intensities for $note_s
...
# Column "ForeGround-BackGround": the signal of the probe after background correction. We use these values for data analysis.~;
    ...
    open( RAW, $raw_files[$i] ) || die "raw.txt NOT ready!";
    $sheet->merge_range(
      2, 5, 2,
      4 + ($#file + 1) * 2,
  ...
  &setformat;
  $note_c = $dn == 1 ? '' : ' & scale normalization for between slide normalization';
  $note = qq~# This page contains an expression matrix for $note_s\n
# Lowess normalization for within slide normalization$note_c.
...
# Column "Normalized value"contains the normalized value of miRNA.~;
  ...
  $note = $dn == 1 ?
    "Box plot for Lowess Normalization
    The box plot shows the miRNA expression profiling before and after normalization.(left: no normalization; right:after slide normalization). The main purpose of lowess normalization is to control within slide variability. Y-axis represents the log2Ratio M=log2(Hy5/Hy3)."
  : "Box plot for Lowess & Scale Normalization
...
sub Rroutine {
  my ($i,@gpr_filtered,@gpr_raw);
  # format file locate varibles for R script
  foreach $i (keys %lot_routes) {
    push(@gpr_raw,"lot$i = c(\"".join('","',map { "../" . $_ } @{$lot_routes{$i}})."\")");
    push(@gpr_filtered,"lot$i = c(\"".join('","',map {s/.*\///;$_} @{$lot_routes{$i}})."\")");
  }
  open( R, ">temp/script.R" ) || die "File output error:$!";
  $pc = 0.05;
  $cf = '2.0';
  print "\n------  请输入Fold Change比较阈值：(默认2.0)  ------\n";
  print "\nPlease input fold change cut-off (default 2.0):";
  $i = <STDIN>;
  if ( $i =~ /(\d\.*\d*)/ and $1 > 1 and $1 < 10 ) { $cf = $1; }
  print "\nSetting fold change cut-off to $cf.\n";
  print SMR "\nSetting fold change cut-off to $cf.\n";
  my $rscript =
q~library(limma,quietly = T)
setwd('temp')
join.RGList <- function (RGlist) 
{
  # inner join a list of RGlist
  # higher version of lot number have priority in genes.Name field
  RG.names <- sort(names(RGlist),decreasing=T)
  x <- RGlist[[RG.names[1]]]
  if (length(RGlist) > 1)
    for (i in 2:length(RGlist)) {
      y <- RGlist[[RG.names[i]]]
      if (!is(y, "RGList")) 
      ...
      uniq_gene1 = makeUnique(genes1)
      uniq_gene2 = makeUnique(genes2)
      uniq_id = intersect(uniq_gene1,uniq_gene2)
      ord1 <- match(uniq_id, uniq_gene1)
      ord2 <- match(uniq_id, uniq_gene2)
      x <- cbind(x[ord1,], y[ord2, ])
    }
  return(x)
}
...
.join(",\n",@gpr_raw)
.q~)
dn = length(unlist(gpr_filtered))
RGlist = list()
raw_RGlist = list()
for (lot_number in names(gpr_filtered)) {
  if(file.exists(lot_number)) {
    # read gpr files
    setwd(lot_number)
    cat(lot_number,"\n")
    RGlist[[lot_number]] <- read.maimages(unlist(gpr_filtered[lot_number]),
                                          "genepix.median")
    setwd("../")
    raw_RGlist[[lot_number]] <- read.maimages(unlist(gpr_raw[lot_number]),
                                              "genepix.median")
    # output raw data for each array
    raw_sub = backgroundCorrect(raw_RGlist[[lot_number]], method="subtract")
    colnames(raw_sub$R) <- paste(colnames(RGlist[[lot_number]][["R"]]),"Hy5")
    colnames(raw_sub$G) <- paste(colnames(RGlist[[lot_number]][["G"]]),"Hy3")
    write.table(cbind(raw_sub$genes,raw_sub$R,raw_sub$G),
                file=paste("raw_",lot_number,".txt",sep = ""),
                sep="\t",quote=FALSE,row.names=FALSE,col.names=TRUE)
  }else{
    stop(c(lot_number," dir not exists!"))
  }
}

RGraw <- join.RGList(raw_RGlist)
RGb <- backgroundCorrect(RGraw, method="subtract")
RGb$R[RGb$R<0] <- 0
RGb$G[RGb$G<0] <- 0

RG <- join.RGList(RGlist)
RGr <- (RG$R-RG$Rb) / (RG$G-RG$Gb)
...
cat("filter miRNAs without 2 or more valid spots\n")
for (i in unique(MAa$genes[,"ID"])){
  for (j in colnames(MAa$M)){
    if(length(MAa$M[MAa$genes[,"ID"]==i&!is.na(MAa$M[,j]),j]) < 2){
    # 有效值少于2个
      MAa$M[MAa$genes[,"ID"]==i,j] <- NA
      MAa$A[MAa$genes[,"ID"]==i,j] <- NA
      RGr[MAa$genes[,"ID"]==i,j] <- NA
    }
  }
}

cat("merge miRNA by ID ...\n")
for (i in sort( unique(MAa$genes[,"ID"]) ) ){
  if(dn > 1){ # multi-sample
    ...
    l_before <- rbind(l_before,log2(apply(RGr[MAa$genes[,"ID"]==i,],2,median,na.rm=T)));
  }else{ # one sample
    ...
    l_before <- rbind(l_before,log2(median(RGr[MAa$genes[,"ID"]==i,],na.rm=T)));
  }
...
cat("scale normalization between arrays ...\n")
ma <- list(M=m,A=a)
...
cat("do miRNA expression profiling ...\n")
~ 
    . $rfold 
    . $rpv 
    . $rmean 
...
cat("compute all differentially expressed miRNAs ...\n")
' . $rdiff . '
cf <- ' . $cf . '
pc <- ' . $pc . '
cfs <- "' . $cf . '"
...
for (j in colnames(fold)){
  ...
  if(exists("up")){
    up[is.na(up)] <- ""
    write.table(up,file=paste(j,cfs,"up.txt",sep="_"),sep="\t",
        na="",quote=FALSE,row.names=FALSE,col.names=TRUE)
    rm(up)
  }
  if(exists("down")){
    down[is.na(down)] <- ""
    write.table(down,file=paste(j,cfs,"down.txt",sep="_"),
  ...
  if(exists("pv")){
    if(exists("uppv")){
    uppv[is.na(uppv)] <- ""
      write.table(uppv,file=paste(j,cfs,"uppv.txt",sep="_"),sep="\t",
          na="",quote=FALSE,row.names=FALSE,col.names=TRUE)
      rm(uppv)
    }
    if(exists("downpv")){
    downpv[is.na(downpv)] <- ""
      write.table(downpv,file=paste(j,cfs,"downpv.txt",sep="_"),sep="\t",
          na="",quote=FALSE,row.names=FALSE,col.names=TRUE)
      rm(downpv)
    }
...
boxplot(l_before,ylab="Log2Ratio(Hy5/Hy3)",main="Before Normalization",
    pch=4,cex=.5,border=rainbow(dn),outline=T,names=F)
text(1:dn, par("usr")[3] - dev(l_before)*0.03, srt = 30, adj = 1,
    labels = names, xpd = T, font = 1)
if (dn > 1) {
  boxplot(l,ylab="Log2Ratio(Hy5/Hy3)",main="After LOWESS & scale Normalization"
          ,pch=4,cex=.5,border=rainbow(dn),outline=T,names=F)
}else{
  boxplot(l,ylab="Log2Ratio(Hy5/Hy3)",main="After LOWESS Normalization"
          ,pch=4,cex=.5,border=rainbow(dn),outline=T,names=F)
}
text(1:dn, par("usr")[3] - dev(l)*0.03, srt = 30, adj = 1,
    labels = names, xpd = T, font = 1)
dev.off()

cat("draw scatter, MA plot ...\n")
...
for (i in names){
  ...
  plot(iG[,i],iR[,i],xlim=c(10,100000),ylim=c(10,100000),xlab="Hy3 Intensity",ylab="Hy5 Intensity",
      ...
      cex.main=1,cex.lab=1,cex.axis=1,mgp = c(2, .6, 0))
...
cat("write data for clustering ...\n")
write.table(cbind(idname[apply(!is.na(l),1,any),2],
    l[apply(!is.na(l),1,any),]),file="cluster.txt",
    na = "", quote = F, row.names = F, sep = "\t",
    col.names=c("name",names))
';
  print R $rscript;
  close R;

  if ( system("Rscript temp/script.R -q") == 0 ) {
...
sub folder_information {
  ...
  foreach (@subfiles) {
    ...
    if ( !/image\/|result\/|temp\// && /\/(.+?)_R1\.jpg$/i ) {
      copy( $_, "./image/$1.jpg" );
    }
  }
  $dn = @sn;
...
sub filtergpr {
  my ( $flag, @data, $line, $file, $s, %lotNum, $gpr_header );
  my $n = @routes;
  foreach $s (0 .. $#routes) {
    $routes[$s] =~ m/\/(.+?\.gpr)$/;
    $file = $1;
    open( IN,  $routes[$s] ) || die "can't open file:$!";
    $flag = 0;
    $gpr_header = '';
    while (<IN>) {
      if ( $flag == 1 ) {
        ...
        if (/^"GalFile=.+lot(\d+)-\d+.+gal"/) {
          $lotNum{$s} = $1;
          say SMR "\t$sn[$s] lot number:\t$lotNum{$s}";
          push(@{$lot_routes{$lotNum{$s}}},$routes[$s]);
          mkdir("temp/lot$lotNum{$s}");
          open( OUT, ">temp/lot$lotNum{$s}/$file" ) || die "$!";
        }
        if (/^"ImageFiles=.+\\(.+)\\.+\.tif/) {
          $tifdir = $1;
        }
