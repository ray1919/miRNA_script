...
for (i in sort( unique(MAa$genes[,"ID"]) ) ){
  if(length(files) > 1){ # multi-sample
    m <- rbind(m,apply(MAa$M[MAa$genes[,"ID"]==i,],2,median,na.rm=TRUE));
    a <- rbind(a,apply(MAa$A[MAa$genes[,"ID"]==i,],2,median,na.rm=TRUE));
    ...
    m <- rbind(m,median(MAa$M[MAa$genes[,"ID"]==i,],na.rm=TRUE));
