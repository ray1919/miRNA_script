...
open(SMA, ">result/summary.txt") || die $!;
print SMA "Code verstion: v2011-12-14\n";
print SMA ctime(),"\n";
...
my ( @raw, $rn, $book, $sheet, $format, @format );
&read_data;
...
qq~# This page contains raw intensities for all $dn slides.\n# Column "Block": block number of each probe.\n# Column "Column": column number of each probe.\n# Column "Row": row number of each probe.\n# Column "Name": the name of each miRNA/probe.\n# Column "ID": array ID of the probes, each miRNA always has its unique probe, but some miRNAs may have two different probes.\n# Column "ForeGround": the foreground intensity of each probe.\n# Column "ForeGround-BackGround": the signal of the probe after background correction.~;
  ...
  &set_format();
  $sheet->set_row( 0, 150 );
  $sheet->merge_range( "A1:I1", $notetext, $format[1] );

  # title
  $sheet->merge_range( 2, 5, 2, 4 + $dn, "ForeGround", $format[2] );
  $sheet->merge_range( 2, 5 + $dn, 2, 4 + 2 * $dn,
    "ForeGround-BackGround", $format[3] );

  my @row = ( "Block", "Column", "Row", "Name", "ID" );
  foreach $s (@sn) { push( @row, decode('Detect',$s) . decode('Detect',"_原始值" )); }
  foreach $s (@sn) { push( @row, decode('Detect',$s) . decode('Detect',"_修正值" )); }
  $sheet->write_row( "A4", \@row, $format[4] );

  # value
  $sheet->set_column( "A:C", 8 );
  $sheet->set_column( "D:D", 18 );
  print "\nSaving <Raw Intensity File.xlsx>\n";
  $sheet->write_col( 4, 0, \@raw, $format[5] );
  $book->close() or die "\nError closing file: $!";
...
\ttreat1,treat2,treat3(treat) vs control1,control2,control3(control)\n\n";
  ...
  if ( $n == 0 ) {
    ...
    print SMA "$n pair(s) will be add to the result.\n-------\n";
  }
  ...
  # 询问是否进行成对样品处理
  if ($is_paired == 1) {
    print "是否作为 成对样品 进行计算p value (y/N):";
    $n = <STDIN>;
    chomp $n;
...
sub expression {
  ...
  my ( $ttest, %cv, @title, $line_pv, $line_cv);
...
#  $format = $book->add_format();
  ...
  $line_pv = defined $pc ? "\n# Column \"P value\": T-test result between samples in different groups."
                         : "";
...
NOTE:1. median value of valid probe intensity (background corrected intensity >= 50 in all samples).
  ...
  # title
  @title = map {decode('Detect', $_)} @title;
    ...
    )
  {
    ...
    $sheet->write_row( $row, 0, \@line, $format[5] );
...
# When two samples are the same, the correlation coefficient R of them reflects the reproducibility of the slides.~;
  ...
  # title
  $sheet->write( "A3", "Correlation coefficient matrix", $format[4] );

  # correlarion
  @line = &cor_cal;
  $sheet->write_col( "A4", \@line, $format[5] );
...
Eisen MB, Spellman PT, Brown PO and Botstein D. Cluster Analysis and Display of Genome-Wide Expression Patterns. Proc Natl Acad Sci USA 1998; 95: 14863-14868.~;
  ...
  # note
  $sheet->set_row( 0, 160 );
  $sheet->merge_range( "A1:O1", $notetext, $format[1] );
...
abline(h = mean(median(log2(before), na.rm = T)))
text(1:$dn, par("usr")[3] - 0.3, srt = 60, adj = 1,
    labels = colnames(before), xpd = T, font = 2)
...
text(1:$dn, par("usr")[3] - 0.3, srt = 60, adj = 1,
    labels = colnames(before), xpd = T, font = 2)
dev.off()~;
  system("R <temp/R_boxplot.R --vanilla -q");
...
sub differential {
  ...
  my ( $bookpv, $sheetpv, $n3, $n4, $line_pv, $line_cv, $pairn_str );
  if ( defined $pc ) {
  ...
  for ( $p = 0 ; $p < @pairs ; $p++ ) {
    ...
    $s2         = $2;
    @pv_cluster = ();
    $pairn_str = decode('Detect',$pairn[$p]);
    ...
    @titledi = map {decode('Detect', $_)} @titledi;
    ...
    $sheet = $book->add_worksheet( $pairn_str );
    ...
    $line_cv = defined $pc ? "\n# Column \"CV-value\": coefficient of variation between samples in different groups."
                           : "";
    $notetext = qq~# Condition pairs: $pairn_str
...
NOTE: The low intensity differentially expressed  miRNAs are filtered in the following list (miRNAs that ForeGround-BackGround intensities < 50 in two samples are filtered). If you are interested in these miRNAs, you can find them in sheet "Expression Matrix" of miRNA Expression Profiling Data.xls file.~;
    $sheet->set_row( 0, defined $pc ? 180 : 160);
    $sheet->merge_range( "A1:N1", $notetext, $format[1] );

    # up
    $sheet->merge_range( 2, 0, 2, $#titledi,
      "$pairn_str $cf fold up regulated miRNAs", $format[6] );
    ...
    $sheet->write_col( 5, 0, \@up, $format[5] );

    # down
    $sheet->merge_range(
      6 + scalar @up,
      0, 6 + scalar @up,
      $#titledi, "$pairn_str $cf fold down regulated miRNAs", $format[7] );
    if ( exists $groups{$s1} and defined $pc ) {
    ...
    $sheet->write_col( 5 + $#up + 5, 0, \@down, $format[5] );

    if ( defined $pv{$id}[$p] ) {
      $sheetpv = $bookpv->add_worksheet( $pairn_str );
...
NOTE: The low intensity differentially expressed  miRNAs are filtered in the following list (miRNAs that ForeGround-BackGround intensities<50 in two samples are filtered). If you are interested in these miRNAs, you can find them in sheet "Expression Matrix" of miRNA Expression Profiling Data.xls file.~;
      ...
      $sheetpv->merge_range( 2, 0, 2, $#titledi,
        "$pairn_str $cf fold up regulated miRNAs", $format );
      if ( exists $groups{$s1} ) {
      ...
      $sheetpv->merge_range(
        6 + scalar @pvup,
        0, 6 + scalar @pvup,
        $#titledi, "$pairn_str $cf fold down regulated miRNAs",$format);
      if ( exists $groups{$s1} ) {
...
dev.off()~;
    system("R <temp/volcano.R --vanilla -q");
...
sub cluster {
  ...
  close OUT;
  print "cluster.txt for clustering is placed in temp dir.\n";
}
...
sub cor_cal {
  ...
  for ( $i = 0 ; $i < $dn ; $i++ ) {
    for ( $j = $i + 1 ; $j < $dn ; $j++ ) {
      ...
      $flag = 0;
      if ( defined $pc && $gn{ $sn[$i] } eq $gn{ $sn[$j] } ) {
        # 多组芯片则输出组内两两比较散点图
        $flag = 1;
      }
      elsif ( !defined $pc ) {
        for ( $k = 0 ; $k < @pairn ; $k++ ) {
          # 非多组芯片则至输出定义的比较散点图
          if ( $pairn[$k] eq "$sn[$i] vs $sn[$j]"
              || $pairn[$k] eq "$sn[$j] vs $sn[$i]" ) {
            $flag = 1;
            last;
          }
          # 多个比一个情况，无p value 有分组
          if ( ( $#{$pairs[$k][0]} > 0 && $#{$pairs[$k][1]} == 0 )
              || ( $#{$pairs[$k][0]} == 0 && $#{$pairs[$k][1]} > 0 ) ) {
            $flag = 1;
            last;
          }
        }
    ...
    for ( $j = $i ; $j >= 0 ; $j-- ) {
      $cor[0][ $j + 1 ] = decode('Detect',$sn[$j]);
      $cor[ $i + 1 ][ $j + 1 ] = $i == $j ? 1 : $cor[ $j + 1 ][ $i + 1 ];
    }
    $cor[ $i + 1 ][0] = decode('Detect',$sn[$i]);
  }
  $cor[0][0] = '';
  ...
  if ( defined $pc ) {
    ...
    for ( $i = 0 ; $i < $gn ; $i++ ) {
      ...
      for ( $j = $i ; $j >= 0 ; $j-- ) {
        $corg[0][ $j + 1 ] = decode('Detect',$gn[$j]);
        $corg[ $i + 1 ][ $j + 1 ] = $i == $j ? 1 : $corg[ $j + 1 ][ $i + 1 ];
      }
      $corg[ $i + 1 ][0] = decode('Detect',$gn[$i]);
    }
    $corg[0][0] = '';
    push( @cor, @corg );
  }
...
dev.off()~;
  close OUT;
  system("R <temp/scatter.R --vanilla -q");
  return @cor;
}
...
sub get_spike {
  my ( %spike, %sp_array, %cv_array, @corr, $id, $i, $flag, $k, $j, @spike_mean, $v, @sn_decode );
  foreach $id ( keys %{ $mirna{0} } ) {
...
sub set_format {
  $format[1] = $book->add_format();
  $format[1]->set_font('Verdana');
  $format[1]->set_size(10);
  $format[1]->set_bg_color(26);
  $format[1]->set_text_wrap();
  $format[1]->set_align("top");

  $format[2] = $book->add_format();
  $format[2]->set_font("Arial");
  $format[2]->set_bold();
  $format[2]->set_align("center");
  $format[2]->set_size(11);
  $format[2]->set_bg_color(61);

  $format[3] = $book->add_format();
  $format[3]->set_font("Arial");
  $format[3]->set_bold();
  $format[3]->set_align("center");
  $format[3]->set_size(11);
  $format[3]->set_bg_color(13);

  $format[4] = $book->add_format();
  $format[4]->set_font("Arial");
  $format[4]->set_bold();

  $format[5] = $book->add_format();
  $format[5]->set_font("Arial");
  $format[5]->set_align("left");

  $format[6] = $book->add_format();
  $format[6]->set_font('Arial');
  $format[6]->set_align("center");
  $format[6]->set_bold();
  $format[6]->set_bg_color('red');

  $format[7] = $book->add_format();
  $format[7]->set_font('Arial');
  $format[7]->set_align("center");
  $format[7]->set_bold();
  $format[7]->set_bg_color(50);
