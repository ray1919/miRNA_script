...
print SMA "Code version: v2012-06-01\n";
print SMA ctime(),"\n";
...
sub differential {
  ...
  for ( $p = 0 ; $p < @pairs ; $p++ ) {
    ...
    $line_cv = $do_ttest{$pairn[$p]} ? "\n# Column \"CV-value\": coefficient of variation between samples in different groups."
                          : "";
    $line_50 = $do_ttest{$pairn[$p]} ? "miRNAs that ForeGround-BackGround intensities < 50 in all samples of two groups are filtered"
                          : "miRNAs that ForeGround-BackGround intensities < 50 in two samples are filtered";
...
# Column "CV-value": coefficient of variation between samples in different groups.\n
NOTE: The low intensity differentially expressed  miRNAs are filtered in the following list (miRNAs that ForeGround-BackGround intensities < 50 in all samples of two groups are filtered). If you are interested in these miRNAs, you can find them in sheet "Expression Matrix" of miRNA Expression Profiling Data.xls file.~;
