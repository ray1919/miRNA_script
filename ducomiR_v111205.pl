...
sub report {
  ...
  &setformat;
  $note_s = $dn == 1 ? '1 slide.' : "all $dn slides.";
  # Raw Intensity File.xlsx
...
text(1:dn, par("usr")[3] - dev(l_before)*0.03, srt = 60, adj = 1,
...
text(1:dn, par("usr")[3] - dev(l)*0.03, srt = 60, adj = 1,
    labels = names, xpd = T, font = 1)
dev.off()
