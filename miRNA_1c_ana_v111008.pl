...
#     REVISION: 10/08/2011 04:13:47 PM
...
sub read_data {
  my ( $gpr, $flag, @line, $n, $s, @diff, $note );
  $s    = 0;
  $note = "";
  foreach $gpr (@routes) {
    $n = 0;
    ...
    while (<GPR>) {
      if ( $flag == 1 ) {
        ...
        elsif ( $line[3] ne $raw[$n][3] || $line[4] ne $raw[$n][4] ) {
          push( @diff, $n );
          $note .=
            "Block $line[0], Col $line[1], Row $line[2] 不一致，已删除\n";
        }
        $raw[$n][ 4 + $s ] = $line[20];
...
sub differential {
  ...
  my ( $bookpv, $sheetpv, $n3, $n4, $line_pv, $line_cv );
  if ( defined $pc ) {
