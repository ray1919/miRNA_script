...
#     REVISION: 08/24/2011 01:24:42 PM
...
sub expression {
  ...
  foreach $id ( keys %{ $mirna{0} } ) {
    for ( $i = 0 ; $i < @pairs ; $i++ ) {
      ...
      # p-value of t-test between groups
...
# Column "Normalized" means the normalized signal of the miRNA.We use Median Normalization Method to do array normalization.\n
NOTE: The low intensity differentially expressed  miRNAs are filtered in the following list (miRNAs that ForeGround-BackGround intensities < 50 in two samples are filtered). If you are interested in these miRNAs, you can find them in sheet "Expression Matrix" of miRNA Expression Profiling Data.xls file.~;
    ...
    $format->set_align("top");
    $sheet->set_row( 0, 160 );
    $sheet->merge_range( "A1:L1", $notetext, $format );
...
for (name in fl) {
    ...
    points(log2(a[a[, 1] >= 1.5 & a[, 2] <= 0.05, 1]), -log10(a[a[,
        1] >= 1.5 & a[, 2] <= 0.05, 2]), col = 2, pch = 19)
    points(log2(a[a[, 1] <= 2/3 & a[, 2] <= 0.05, 1]), -log10(a[a[,
        1] <= 2/3 & a[, 2] <= 0.05, 2]), col = 2, pch = 19)
    abline(v = log2(c(1.5, 2/3)), lty = "dashed", col = "darkgreen",
        lwd = 2)
    abline(h = -log10(0.05), lty = "dashed", col = "darkgreen", lwd = 2)
}
dev.off()~;
    ...
    $notetext = qq~Volcano Plots\n
      ...
      The vertical lines correspond to 1.5-fold up and down, respectively, and the horizontal line represents a p-value of 0.05. So the red point in the plot represents the differentially expressed genes with statistically significance.\n
...
sub cor_cal {
  my ( @cor, $i, $j, @array1, @array2, $id, $sn, $gn, $n, $k );
  $n = 101;
...
sub write_title {
  ...
  $format->set_text_wrap();
  $format->set_align("center");
  $tsheet->set_row( $row + 1, 15.5 );
  $tsheet->write_row( $row + 1, 0, $title, $format );
}

sub get_spike {
  my ( %spike, %sp_array, %cv_array, @corr, $id, $i, $flag, $k, $j,
    @spike_mean );
  foreach $id ( keys %{ $mirna{0} } ) {
    if ( $mirna{0}{$id}[0] =~ /spike_control_v(\d)/ ) {
      ...
      for ( $i = 0 ; $i < $dn ; $i++ ) {
        if ( $mirna{$i}{$id}[5] < 50 ) {
          ...
          push( @{ $cv_array{$1}{$i} }, $mirna{$i}{$id}[6] );
        }
      ...
      if ( $flag == 1 ) {
        for ( $i = 0 ; $i < $dn ; $i++ ) {
          push( @{ $sp_array{$1}{$i} }, $mirna{$i}{$id}[5] );
        }
      }
    }
  }
  ...
  foreach $i ( 1 .. 2 ) {
    ...
    for ( $j = 0 ; $j < $dn ; $j++ ) {
      $corr[$i][ $j + 1 ][0] = $sn[$j];
      for ( $k = $j ; $k < $dn ; $k++ ) {
        if ( defined $sp_array{$i}{$j}[1] ) {
          $corr[$i][ $j + 1 ][ $k + 1 ] =
            correlation( $sp_array{$i}{$j}, $sp_array{$i}{$k} );
        }
        else {
          $corr[$i][ $j + 1 ][ $k + 1 ] = 'N/A';
        }
  ...
  $sheet->write_row( 1, 0, $spike_mean[1], $format );
  $sheet->write( 2, 0, "Spike v2 median for each slide", $format );

  $sheet->write_row( 3, 0, $spike_mean[2], $format );
  $sheet->write( 5, 0, "Sample correlation using spike v1 F_B median",
    $format );
  $sheet->write_col( 6, 0, $corr[1], $format );
  $sheet->write( $dn + 8, 0, "Sample correlation using spike v2 F_B median",
    $format );
  $sheet->write_col( $dn + 9, 0, $corr[2], $format );
  $k = 14;
  $sheet->merge_range(
    2 * $dn + 12,
    1, 2 * $dn + 12,
  ...
  $sheet->write_row( 2 * $dn + 13, 1,       \@sn, $format );
  $sheet->write_row( 2 * $dn + 13, 1 + $dn, \@sn, $format );
